...
// make these ports anything you like,
// make sure that the dst ports match the ports opened on the server
...
void testSendUDP(UDPsocket udpSocket) {
    ...
    packet->len = sizeof(bool);  // setting the packet length to the size of a bool
...
void receivePlayerStates(UDPsocket udpSocket) {
    ...
    if (SDLNet_UDP_Recv(udpSocket, packet)) {
        playerState state;
        if (packet->len == sizeof(state)) {
            // Copy data from packet to state
            memcpy(&state, packet->data, sizeof(state));

            // Log received data
            SDL_Log("Received position: (%d, %d), Rotation: %lf", state.x, state.y, state.rotation);
